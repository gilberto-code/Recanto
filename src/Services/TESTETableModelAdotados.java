 ...
 * @author Daniela
 */
public class TESTETableModelAdotados extends AbstractTableModel {
    private static final String[] columnNames = 
                    {"Fotuuuuuu", "Nome", "Sexo", "Idade", "Espécie"};
    private ArrayList<Animal> animals;
    private ControllerAnimal dao;
    
    //O construtor já recebe os dados do ArrayList para exibição 
    public TESTETableModelAdotados() throws Exception{
        this.dao = new ControllerAnimal();
        this.animals = this.dao.getList();
        fireTableRowsInserted(0, this.animals.size()-1);  //Força a chamada de getValueAt() para inserir todas as células na JTable
    }
    
    //O construtor já recebe os dados do ArrayList para exibição 

    public TESTETableModelAdotados(ArrayList<Animal> a) {
         //Força a chamada de getValueAt() para inserir todas as células na JTable
    this.animals = a;
        fireTableRowsInserted(0, a.size()-1);
